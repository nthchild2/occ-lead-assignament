...
6. Verification gate       →  typecheck + lint + test, every change, before claiming done
...
No change is reported complete without running, in this order: `tsc --noEmit` for every touched workspace, `eslint`, and `jest`. This isn't aspirational — it's what actually happened in this session, and it caught real, non-trivial problems before they reached a commit:
