...
5. Implementación                →  código de funcionalidades, contra la arquitectura y
                                     las salvaguardas
6. Gate de verificación          →  typecheck + lint + test + expo install --check,
                                    en cada cambio, antes de
...
Ningún cambio se reporta como completo sin ejecutar, en este orden: `tsc --noEmit` para cada workspace tocado, `eslint`, `jest` y `expo install --check` (`pnpm verify` corre los cuatro; CI corre el mismo gate en cada push/PR — ver A4 Decisión 4). Esto no es aspiracional — es lo que realmente sucedió en esta sesión, y detectó problemas reales y no triviales antes de que llegaran a un commit:
...
| `expo install --check`      | **Agregado después de una revisión externa en entorno limpio, no antes.** pnpm resolvió `babel-preset-expo` a v57 y `react-native-worklets` a 0.6.0 — ambos más nuevos que lo que el lado nativo precompilado de Expo Go SDK 54 trae — y la app hacía red-screen en el primer arranque mientras `tsc`/`eslint`/`jest` seguían en verde. La lección: esas tres herramientas nunca ejercitan Metro, Babel ni el runtime nativo, así que la alineación de versiones con el SDK necesita su propio paso de gate. Ahora fijado vía `pnpm.overrides` y aplicado por este check localmente (`pnpm verify`) y en CI |
...
- La última fila es el post-mortem del propio gate: el gate original de tres herramientas tenía un punto ciego, la corrida en clon limpio de un revisor externo lo encontró, y la corrección fue ampliar el gate en lugar de parchear el síntoma — la misma regla de "arregla el proceso, no solo el bug" que este documento existe para hacer cumplir.
